...
const CurrencyConverter = () => {
  ...
  const [conversionResultPKR, setconversionResultPKR] = useState(0);
  ...
  const fetchExchangeRate = async () => {
    try {
      ...
      const exchangeRatePKR = data.rates.PKR;
      const exchangeRateEUR = data.rates.EUR;
      setconversionResultPKR(
        parseFloat(cryptocurrencyPrice) * exchangeRatePKR
      );
        ...
        <View className="w-full h-[65%] p-5">
          ...
          <View className="mt-8 px-5" style={[currencyConverterStyles.card]}>
            ...
            <View className="" style={[ currencyConverterStyles.flexEnd ]}>
              <Text style={[ currencyConverterStyles.priceTitle ]}>Rs {conversionResultPKR.toFixed(2)}</Text>
              <Text style={[ currencyConverterStyles.subTitle ]}>valuation in PKE</Text>
            </View>
          </View>
        </View>
